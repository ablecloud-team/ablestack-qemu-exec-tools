...
Requires:       cloud-init
#Requires:       qemu-guest-agent   # ?„ìš”??ì¶”ê?
...
if _is_ablestack_host; then
    ...
    # ?¬ê¸°??ë°”ë¡œ ì¢…ë£Œ ???„ëž˜ ê¸°ì¡´ guest???‘ì—…?€ ?¤í–‰ ????    exit 0
fi

echo "[INFO] ablestack-qemu-exec-tools post install start (guest VM)"

# agent_policy_fix / cloud_init_auto ?¤í–‰
