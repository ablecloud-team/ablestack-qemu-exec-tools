...
v2k_manifest_init() {
  ...
  # VDDK(vCenter Ï§ëÏã¨) Î©îÌ??∞Ïù¥???§Î≤Ñ?ºÏù¥??
  # - V2K_VDDK_SERVER: vCenter host/ip (Í∂åÏû•)
  # - V2K_VDDK_THUMBPRINT: vCenter SSL SHA1 thumbprint (Í∂åÏû•)
  # - V2K_VDDK_USER: ?†ÌÉù(cred_file ?¥Î???VDDK_USER/VDDK_PASSWORDÍ∞Ä ?§ÏÇ¨??
  # - V2K_VDDK_CRED_FILE: workdir ?òÏúÑ??Î≥µÏÇ¨??vddk.cred Í≤ΩÎ°ú
  ...
  # inventory JSON???®Ïùº JSON Í∞ùÏ≤¥?∏Ï? ?ïÍ∑ú??(?¨Í∏∞??Íπ®Ï?Î©?Ï¶âÏãú ?§Ìå®)
  local inv_compact
  ...
  # ??inv_json??stdin?ºÎ°ú jq???ÑÎã¨ (Í≤ÄÏ¶ùÎêú ?®ÌÑ¥)
  printf '%s' "${inv_compact}" | jq -c \
    ...
    # ?îÏä§??Î≥Ä?? to_entries??{key,value} ?¥Î?Î°?keyÎ•??∏Îç±?§Î°ú ?¨Ïö©
    | ($inv.disks
        | to_entries
        | map(
            . as $e
    ...
    # rbd ?Ä?ÖÏù¥Î©?Í∞?target_pathÍ∞Ä rbd: Î°??úÏûë?¥Ïïº ??
    | if $st=="rbd" then
    ...
    | {
        ...
        source: {
          ...
          # VDDK access (vCenter Ï§ëÏã¨)
          # - server: Í∏∞Î≥∏ vCenter host (override Í∞Ä??
          # - thumbprint: vCenter SSL SHA1 thumbprint (override Í∞Ä??
          # - cred_file: VDDK_USER/VDDK_PASSWORD[/VDDK_SERVER/VDDK_THUMBPRINT] ?¨Ìï®
          vddk: {
...
v2k_manifest_status_summary() {
  ...
  local msum
  msum="$(jq -c '
    # runtime.sync_issues??engine.shÍ∞Ä incr/final guard?êÏÑú Í∏∞Î°ù(?úÌíà??Í¥ÄÏ∏°Ïö©)
    (.runtime // {}) as $rt
    ...
    | {
        ...
        # ?¨Îûå??Î≥¥Í∏∞ ?¨Ïö¥ "?ÅÌÉú ?îÏïΩ ?åÌä∏" (UI/CLI Í≥µÌÜµ ?úÏö©)
        hints:{
...
v2k_manifest_fetch_and_save_base_change_ids() {
  ...
  # Python ?§ÌÅ¨Î¶ΩÌä∏ ?§Ìñâ???ÑÌïú ?òÍ≤ΩÎ≥Ä???§Ï†ï (Govc ?òÍ≤ΩÎ≥Ä???úÏö©)
  ...
  for ((i=0;i<count;i++)); do
    ...
    # --change-id "*"Î•??òÍ≤®???ÑÏû¨ ?úÏ†ê??Change ID (new_change_id)Îß?Î∞õÏïÑ?®Îã§.
    ...
    if [[ -n "${new_id}" && "${new_id}" != "null" ]]; then
      # Ï°∞Ìöå??IDÎ•?last_change_idÎ°??Ä??(?§Ïùå Incr??Í∏∞Ï??êÏù¥ ??
