...
v2k_target_generate_libvirt_xml() {
  ...
  if [[ "${fw}" == "efi" ]]; then
    ...
    # Î∞©Ïñ¥(?ÑÏàò): ÎπÑÏñ¥?àÏúºÎ©?Ï¶âÏãú ?§Ìå®
...
v2k_target_set_uefi_secureboot() {
  ...
  if [[ "${on}" == "1" ]]; then
    ...
    vars="${base}/OVMF_VARS.fd"
    secure_attr="no"
    firmware_block=""  # WinPE?? firmware feature block ?úÍ±∞(Í∞Ä???∏Ìôò??Ï¢ãÏùå)
  fi
...
v2k_target_set_boot_cdrom_only() {
  ...
  # 2) <os> ?¥Î???<boot dev='cdrom'/> 1Í∞úÎßå ?ΩÏûÖ
  #    - <type ...> Î∞îÎ°ú ?§Ïóê ?£Îäî??
...
v2k_target_set_boot_hd() {
  ...
  # 2) <os> ?¥Î???<boot dev='hd'/> 1Í∞úÎßå ?ΩÏûÖ
  perl -0777 -i -pe "s#(<os[^>]*>\s*<type[^>]*>[^<]*</type>)#\$1\n    <boot dev='hd'/>#s" \
    "${tmp}.mid"
...
# CDROM ?ÑÎ≥¥: sdv..sdz -> sdaa..sdaz -> sdba..sdbz (?ïÎ∞©??
...
# virsh dumpxml?êÏÑú ?ÑÏû¨ ?¨Ïö©Ï§?target dev Î™©Î°ù Ï∂îÏ∂ú
...
# ?¥Î≤à run?êÏÑú ?¥Î? ?†Îãπ??cdrom dev Í∏∞Î°ù ?åÏùº(?§ÌîÑ Î∞òÏòÅ ÏßÄ?∞ÎèÑ Î∞©Ïñ¥)
...
v2k_target_attach_cdrom() {
  ...
  # ??SATAÎ°?Í∞ïÏ†ú (?úÎùº?¥Î≤Ñ Ï§ëÎ¶Ω) + persistent
