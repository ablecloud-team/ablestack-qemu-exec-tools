...
v2k_nbd_alloc() {
  ...
  for i in $(seq 0 63); do
    ...
    # 1. ?ìž??Atomic) ?ˆì•½ ?œë„: mkdir
    if mkdir "${lock_dir}" 2>/dev/null; then
      # ?ˆì•½ ?±ê³µ! ??PID ê¸°ë¡
      echo "$$" > "${pid_file}"
      
      # 2. ì»¤ë„ ?íƒœ ?´ì¤‘ ì²´í¬ (?´ë? ?¬ìš© ì¤‘ì¸ì§€)
      if v2k_nbd_is_in_use "${dev}" || v2k_nbd_has_dm_children "${dev}"; then
        # ?¬ìš© ì¤‘ì´ë©??ˆì•½ ì·¨ì†Œ?˜ê³  ?¤ìŒ?¼ë¡œ
        rm -rf "${lock_dir}"
        continue
      fi
      ...
      # ?ˆì•½ ?¤íŒ¨ (?´ë? ?½ì´ ì¡´ìž¬??. ì¢€ë¹??½ì¸ì§€ ?•ì¸
      if [[ -f "${pid_file}" ]]; then
        owner_pid="$(cat "${pid_file}" 2>/dev/null || true)"
        # ?„ë¡œ?¸ìŠ¤ê°€ ì£½ì—ˆê³?$owner_pid ?†ìŒ) + ì»¤ë„?ì„œ???¬ìš© ????-> ì¢€ë¹???
        if [[ -n "${owner_pid}" ]] && ! kill -0 "${owner_pid}" 2>/dev/null; then
           if ! v2k_nbd_is_in_use "${dev}"; then
             # ì¢€ë¹???ì²?†Œ ???¬ì‹œ??
             rm -rf "${lock_dir}"
             # ?´ë²ˆ ë£¨í”„(i)ë¥??¤ì‹œ ?œë„?˜ê¸° ?„í•´ ië¥??˜ë‚˜ ì¤„ì´ê±°ë‚˜, ê·¸ëƒ¥ ?¤ìŒ?¼ë¡œ ?˜ì–´ê°?
             # ?ˆì „?˜ê²Œ ?¤ìŒ ?”ë°”?´ìŠ¤ ?ìƒ‰ (?¤ìŒ ?¤í–‰ ?œì—” ì²?†Œ?˜ì–´ ?ˆì„ ê²ƒìž„)
           fi
        fi
      fi
    fi
...
v2k_nbd_free() {
  ...
  if [[ -d "${lock_dir}" ]]; then
     # ???½ì¸ì§€ ?•ì¸?˜ê³  ì§€?°ëŠ” ê²ƒì´ ê°€???ˆì „?˜ì?ë§? 
     # ë³´í†µ freeë¥??¸ì¶œ?˜ëŠ” ê±?ì£¼ì¸?´ë?ë¡?ê°•ì œ ?? œ
     rm -rf "${lock_dir}"
  fi
}
