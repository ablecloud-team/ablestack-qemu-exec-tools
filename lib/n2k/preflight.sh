...
n2k_detect_host_dependencies() {
  ...
  for name in jq curl qemu-img qemu-nbd virsh; do
    ...
    | {
        ...
        steps: (
          if ($pf.selected_mode == "v4-incremental" and $pf.can_run) then
            ["init","preflight","inventory","recovery-point-base","sync-base","recovery-point-incr","sync-incr","shutdown-source","recovery-point-final","sync-final","define-target","cleanup"]
          elif ($pf.selected_mode == "legacy-cbt" and $pf.can_run) then
            ["init","preflight","inventory","legacy-base","legacy-incr","shutdown-source","legacy-final","define-target","cleanup"]
          elif ($pf.selected_mode == "cold-export" and $pf.can_run) then
            ["init","preflight","inventory","shutdown-or-snapshot-source","sync-full-disk","define-target","cleanup"]
